...
class Zefoy:
	if os.path.exists('setting.json') is False: open('setting.json','w',encoding='utf-8',errors='ignore').write(json.dumps({'url':'https://www.tiktok.com/t/ZTRToxYct','service':'Views'},indent=4))
	...
	def __init__(self):
		...
		self.url = 'None'
		self.text = 'By @WHI3PER'
	def get_captcha(self):
		if os.path.exists('sesi'): self.session.cookies.set("PHPSESSID", open('sesi',encoding='utf-8').read(), domain='zefoy.com')
	...
	def send_captcha(self, new_session = False):
		if new_session: self.session = requests.Session(); os.remove('sesi'); time.sleep(2)
		...
		if 'Enter Video URL' in request.text: 
			...
			open('sesi','w',encoding='utf-8').write(self.session.cookies.get('PHPSESSID'))
	...
	def find_video(self):
		...
		while True:
			...
			elif 'service is currently not working' in self.video_info: return (True,'Service is currently not working, try again later. | You can change it in setting.')
	...
	def use_service(self):
		...
		elif 'service is currently not working' in res: return ('Service is currently not working, try again later. | You can change it in setting.')
	...
	def get_video_id(self, url_ = None, set_url=True):
		...
		if request.text == '': print('Invalid URL | Replace in setting'); return False
		else: json_=request.json()
		if 'author' not in json_: print(f'{self.url}| invalid URL | Replace in setting'); return False
	...
	def check_setting(self):
		while True:
			try: 
				last_url = self.url
				setting = json.loads(open('setting.json',encoding='utf-8',errors='ignore').read())
				self.url = setting['url']
				self.service = setting['service']
	...
	def change_setting(self):
		open('setting.json','w',encoding='utf-8',errors='ignore').write(json.dumps({'url':self.url,'service':self.service},indent=4))
	...
	def update_name(self):
		while True:
			try:
				...
				self.text = f"By @WHI3PER | Views: {video_info['viewCount']} | Likes: {video_info['likeCount']} | Comments: {video_info['commentCount']} | Shares: {video_info['shareCount']}"
			except: pass
			time.sleep(5)

Z = Zefoy()
threading.Thread(target=Z.check_setting).start()
threading.Thread(target=Z.update_name).start()
Z.send_captcha()
Z.get_table()
while True:
	try: 
		if 'Service is currently not working, try again later' in str(Z.use_service()): print('Service is currently not working, try again later. | You can change it in setting.');time.sleep(5)
